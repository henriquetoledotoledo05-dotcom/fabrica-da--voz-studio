...
function App() {
  ...
  /*
    IMPORTANTE:
    Estes s├úo os IDs que o projeto j├í estava usando.
  ...
  */
  ...
  const mixar = async () => {
    ...
    try {
      ...
      if (!vozResponse.ok) {
        throw new Error('N├úo foi poss├¡vel acessar a voz gerada.')
      }
      ...
      alert(
        erro instanceof Error
          ? erro.message
          : 'N├úo foi poss├¡vel mixar a voz com a trilha.'
      )
    } finally {
      setMixando(false)
    }
  ...
  const categorias = [
    {
      emoji: '­ƒô╗',
      titulo: 'Spot de R├ídio',
      descricao: 'Crie spots profissionais para sua programa├º├úo',
    },
    {
      emoji: '­ƒÄë',
  ...
  if (categoriaSelecionada) {
    return (
      <div className="app">
        <header>
          <h1>F├üBRICA DA VOZ</h1>
          <p style={{ marginTop: '25px' }}>
            Seu est├║dio de locu├º├úo profissional
          </p>
        </header>

        <main className="main">
          <section className="hero">
            <span className="tag">EST├ÜDIO DE LOCU├ç├âO</span>
            <h2>{categoriaSelecionada}</h2>
            <p>
              Crie sua locu├º├úo profissional com intelig├¬ncia artificial.
            </p>
          </section>

          <section className="categorias">
            <div className="editor">
              <h3>Crie sua locu├º├úo</h3>
              ...
              <div className="estilos">
                <label>Escolha o estilo da locu├º├úo</label>

                <div className="opcoes-estilo">
                  {[
                    ['normal', '­ƒÄÖ´©Å', 'Normal', 'Voz natural e profissional'],
                    ['animado', '­ƒöÑ', 'Animado', 'Mais energia e entusiasmo'],
                    ['muitoAnimado', '­ƒÜÇ', 'Muito Animado', 'Mais ritmo e empolga├º├úo'],
                    ['superImpacto', '­ƒÆÑ', 'Super Impacto', 'Forte, intenso e marcante'],
              ...
              <label>Texto da locu├º├úo</label>

              <textarea
                className="campo-texto"
                value={texto}
                onChange={(e) => setTexto(e.target.value)}
                placeholder="Digite aqui o texto que voc├¬ quer transformar em voz..."
              />

              <div className="editor-tempo">
                <h3>ÔÅ▒´©Å Editor de tempo</h3>

                <p>
                  Defina os segundos de trilha antes e depois da locu├º├úo.
                </p>

                <div className="tempo-opcoes">
                  <div className="tempo-campo">
                    <label>ÔÅ«´©Å In├¡cio</label>
                ...
                <div className="resumo-tempo">
                  ­ƒÄÁ Trilha: <strong>{segundosInicio}s</strong>
                  {' ÔåÆ '}
                  ­ƒÄÖ´©Å Voz
                  {' ÔåÆ '}
                  <strong>{segundosFinal}s</strong> ­ƒÄÁ
                </div>
              </div>

              <div className="controles-mixagem">
                <h3>­ƒÄÜ´©Å Controle da mixagem</h3>
                ...
                <div className="controle">
                  <label>
                    ­ƒîà Fade final: <strong>{fadeFinal}s</strong>
                  </label>
                  <input
                    type="range"
                    min="0"
              ...
              <div className="acoes-geracao">
                <button
                  ...
                  disabled={gerando}
                >
                  {gerando ? 'ÔÅ│ Gerando...' : '­ƒöè Gerar voz'}
                </button>
                ...
                {vozGeradaUrl && (
                  ...
                  >
                    ­ƒÄÁ Adicionar trilha
                  </button>
                )}

                {mostrarTrilhas && (
                  <div className="painel-trilhas">
                    <h3>­ƒÄÁ Escolha uma trilha</h3>
                    ...
                    {trilhas.map((trilha) => (
                      <div className="trilha-item" key={trilha.caminho}>
                        ...
                        >
                          Ô£ô Usar esta trilha
                        </button>
                      </div>
                    ))}

                    <div className="trilha-item">
                      <strong>­ƒôü Enviar minha pr├│pria trilha</strong>
  ...
  return (
    <div className="app">
      <header className="topo">
        <div className="logo">
          <img src={logoFabrica} alt="F├íbrica da Voz" />
        </div>
      </header>

      <main className="main">
        <section className="hero">
          <span className="tag">INTELIG├èNCIA ARTIFICIAL</span>
          ...
          <p>
            Crie spots, chamadas e propagandas com vozes realistas para r├ídio,
            eventos e empresas.
          </p>
        </section>

        <section className="categorias">
          <h3>O que voc├¬ quer produzir?</h3>
      ...
      <footer>
        <p>┬® 2026 F├íbrica da Voz ÔÇó Est├║dio de Locu├º├úo com IA</p>
      </footer>
    </div>
  )
}
